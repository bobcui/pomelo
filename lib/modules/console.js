  ...
  switch(msg.signal) {
    ...
    case 'list':
      ...
      usage.lookup(pid, {keepHistory: true}, function(err, result) {
        ...
        var remoteQueueLength=null
        var remote = app.components.__remote__;
        if (!!remote) {
            remoteQueueLength = remote.remote.getHandleQueueLength()
        }

        var serverQueueLength=null
        var server = app.components.__server__;
        if (!!server) {
            serverQueueLength = server.server.getHandleQueueLength()
        }        
        ...
        utils.invokeCallback(cb, {
          ...
          body: {
            ...
            remoteQueueLength: remoteQueueLength,
            serverQueueLength: serverQueueLength
          }
        });
      });
      break;
...
var checkPort = function(server, cb) {
  ...
  if (!utils.isLocal(host)) {
    ...
    cmd = 'ssh ' + host + ' ' + ssh_params.join(' ') + ' ' + cmd;
  }
...
var parseArgs = function(msg, info, cb) {
  ...
  for(var i =0; i<args.length; i++) {
    ...
    var key = args[i].substring(0, index)
    rs[key] = args[i].substring(index+1)
    if(!!info[key]) {
      info[key] = 1;
    }
  }
