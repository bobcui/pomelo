...
var SessionService = function(opts) {
  ...
  this.bindTimeout = opts.bindTimeout;
  if (!!this.bindTimeout) {
    this.bindTimeout *= 5
    this.sessionTimeQueue = []
    setInterval(this.closeTimeoutUnbindSession.bind(this), 200)
  }
};

module.exports = SessionService;

SessionService.prototype.closeTimeoutUnbindSession = function() {
  if (!this.bindTimeout) {
    return
  }
  var timeoutTime = Math.floor(Date.now() / 200) - this.bindTimeout
  while(!!this.sessionTimeQueue[0] && this.sessionTimeQueue[0].time <= timeoutTime) {
    var timeSessions = this.sessionTimeQueue.shift()
    for (var i=0; i<timeSessions.sids.length; ++i) {
      var session = this.get(timeSessions.sids[i])
      if (!!session && session.uid === null) {
        session.closed('bindtimeout')
      }
    }
  }
}
...
SessionService.prototype.create = function(sid, frontendId, socket) {
  ...
  if (!!this.sessionTimeQueue) {
    this.addToTimeQueue(session.id)
  }
  return session;
};

SessionService.prototype.addToTimeQueue = function(sid) {
  var timeSessions = this.sessionTimeQueue[this.sessionTimeQueue.length-1]
  var now = Math.ceil(Date.now() / 200)
  if (!timeSessions || timeSessions.time !== now) {
    timeSessions = {
      time: now,
      sids: [sid]
    }
    this.sessionTimeQueue.push(timeSessions)
  }
  else {
    timeSessions.sids.push(sid)
  }
}
 ...
 * @api private
 */
SessionService.prototype.bind = function(sid, uid, cb, sync) {
 ...
 * @api private
 */
SessionService.prototype.unbind = function(sid, uid, cb, sync) {
  var session = this.sessions[sid];
...
SessionService.prototype.kickAll = function(cb) {
    var sessions = {}, sid
    for (sid in this.sessions) {
        sessions[sid] = this.sessions[sid]
    }

    for (sid in sessions) {
        sessions[sid].closed('kick');
    }
...
Session.prototype.isValid = function() {
  return !!this.__sessionService__.get(this.id);
};

Session.prototype.getClientAddress = function() {
...
FrontendSession.prototype.bind = function(uid, cb, sync) {
  var self = this;
...
FrontendSession.prototype.unbind = function(uid, cb, sync) {
  var self = this;
...
FrontendSession.prototype.isValid = function() {
  return !!this.__sessionService__.get(this.id);
};

FrontendSession.prototype.closed = function(reason) {
  this.__session__.closed(reason)
};

FrontendSession.prototype.getClientAddress = function() {
  return this.__session__.getClientAddress()
}
