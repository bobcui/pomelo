 ...
 */
 Application.start = function(cb) {
  logger.info('server %s start', this.serverId)
  ...
  appUtil.startByType(self, function() {
    ...
    if(!!beforeFun) {
      beforeFun(self, function(err){
        if (!!err) {
          logger.error('before startup function error err=%j', err);
        }
        else {
          startUp();
        }
...
Application.stop = function(force) {
  ...
  this.stopTimer = setTimeout(function() {
    logger.info('server %s stopped', self.serverId)
    process.exit(0);
  ...
  var shutDown = function() {
    appUtil.stopComps(self.loaded, 0, force, function() {
      cancelShutDownTimer();
      if(force) {
        logger.info('server %s stopped force', self.serverId)
        process.exit(0);
      }
    });
  };
...
Application.removeServers = function(ids) {
  ...
  this.event.emit(events.REMOVE_SERVERS, ids, items);
};
...
Application.registerLifecycleFunc = function(key, func) {
  var next = this.lifecycleCbs[key];
  if (!!next) {
    this.lifecycleCbs[key] = function(self, cb){
      func(self, function(err){
        if (!!err) {
          cb(err);
        }
        else {
    ...
    this.lifecycleCbs[key] = func;
  }
};

Application.registerBeforeStartupFunc = function(func) {
  this.registerLifecycleFunc(Constants.LIFECYCLE.BEFORE_STARTUP, func)
};

Application.registerBeforeShutdownFunc = function(func) {
  this.registerLifecycleFunc(Constants.LIFECYCLE.BEFORE_SHUTDOWN, func)
};

Application.registerAfterStartupFunc = function(func) {
  this.registerLifecycleFunc(Constants.LIFECYCLE.AFTER_STARTUP, func)
};

Application.registerAfterStartallFunc = function(func) {
  this.registerLifecycleFunc(Constants.LIFECYCLE.AFTER_STARTALL, func)
};
