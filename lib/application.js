...
Application.setLifecycleFunc = function(key, cb) {
  if (this.lifecycleCbs[key] !== undefined) {
    logger.warn('lifecycleCbs[%s] overlapped', key)
  }
  this.lifecycleCbs[key] = cb
};

Application.setBeforeStartupFunc = function(cb) {
  this.setLifecycleFunc(Constants.LIFECYCLE.BEFORE_STARTUP, cb)
};

Application.setBeforeShutdownFunc = function(cb) {
  this.setLifecycleFunc(Constants.LIFECYCLE.BEFORE_SHUTDOWN, cb)
};

Application.setAfterStartupFunc = function(cb) {
  this.setLifecycleFunc(Constants.LIFECYCLE.AFTER_STARTUP, cb)
};

Application.setAfterStartallFunc = function(cb) {
  this.setLifecycleFunc(Constants.LIFECYCLE.AFTER_STARTALL, cb)
};
