 ...
 */
 Application.start = function(cb) {
  logger.fatal('server %s start', this.serverId)
  ...
  appUtil.startByType(self, function() {
    ...
    if(!!beforeFun) {
      async.each(beforeFun, function(fun, cb){
        fun.call(null, self, cb);
      }, function(err){
        if (!err) {
          startUp();
        }
...
Application.afterStart = function(cb) {
  ...
  appUtil.optComponents(this.loaded, Constants.RESERVED.AFTER_START, function(err) {
    ...
    logger.fatal('%j startup in %s ms', id, usedTime);
...
Application.stop = function(force) {
  ...
  this.stopTimer = setTimeout(function() {
    logger.fatal('server %s stopped', self.serverId)
    process.exit(0);
  ...
  var shutDown = function() {
    appUtil.stopComps(self.loaded, 0, force, function() {
      cancelShutDownTimer();
      if(force) {
        logger.fatal('server %s stopped force', self.serverId)
        process.exit(0);
      }
    });
  };
...
Application.registerLifecycleFunc = function(key, cb) {
  if (this.lifecycleCbs[key] === undefined) {
    this.lifecycleCbs[key] = []
  }
  this.lifecycleCbs[key].push(cb)
};

Application.registerBeforeStartupFunc = function(cb) {
  this.registerLifecycleFunc(Constants.LIFECYCLE.BEFORE_STARTUP, cb)
};

Application.registerBeforeShutdownFunc = function(cb) {
  this.registerLifecycleFunc(Constants.LIFECYCLE.BEFORE_SHUTDOWN, cb)
};

Application.registerAfterStartupFunc = function(cb) {
  this.registerLifecycleFunc(Constants.LIFECYCLE.AFTER_STARTUP, cb)
};

Application.registerAfterStartallFunc = function(cb) {
  this.registerLifecycleFunc(Constants.LIFECYCLE.AFTER_STARTALL, cb)
};
