...
var getConnector = function(app, opts) {
  ...
  return connector(curServer.clientPort, curServer.clientHost, opts);
};

var getDefaultConnector = function(app, opts) {
  var DefaultConnector = require('../connectors/sioconnector');
  var curServer = app.getCurServer();
  return new DefaultConnector(curServer.clientPort, curServer.clientHost, opts);
};
...
var hostFilter = function(cb, socket) {
  ...
  var check = function(list) {
    for(var address in list) {
      ...
      if(exp.test(ip)) {
        socket.disconnect('ip in blacklist');
        return true;
      }
    }
    return false;
...
var bindEvents = function(self, socket) {
  if(self.connection) {
    ...
    if (!!curServer['max-connections']) {
      var statisticInfo = self.connection.getStatisticsCount();
      if(statisticInfo.totalConnCount > curServer['max-connections']) {
        ...
        socket.disconnect('server reach max connections');
        return;
      }
    }
  }
...
var getSession = function(self, socket) {
  ...
  socket.on('disconnect', session.closed.bind(session, 'disconnect'));
  socket.on('error', session.closed.bind(session, 'error'));
  session.on('closed', onSessionClose.bind(null, app));
  session.on('bind', function(uid) {
    logger.debug('session on [%s] bind with uid=%s sid=%s', self.app.serverId, uid, session.id);
    // update connection statistics if necessary
    if(self.connection) {
      self.connection.addLoginedUser(uid, session.id, {
        loginTime: Date.now(),
        uid: uid,
        sid: session.id,
  ...
  session.on('unbind', function(uid) {
    if(self.connection) {
      self.connection.removeLoginedUser(uid, session.id);
    }
