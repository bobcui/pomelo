...
var Socket = function(socket, opts) {
  ...
  this.invalidPackageHandler = opts.invalidPackageHandler;
...
var ondata = function(socket, chunk) {
  ...
  if(socket.state === ST_HEAD && socket.packageBuffer === null && !checkTypeData(chunk[0])) {
    if (!!socket.invalidPackageHandler) {
       socket.invalidPackageHandler(socket, chunk)
    }
    else {
      var chunkPart = null
      var chunkLength = -1
      ...
      logger.error('close the connection with invalid head message, the remote ip is %s && port is %s && message is len=%s, %j', socket._socket.remoteAddress, socket._socket.remotePort, chunkLength, chunkPart);
      socket.close();
    }
  }
