...
var ondata = function(socket, chunk) {
  ...
  if(socket.state === ST_HEAD && socket.packageBuffer === null && !checkTypeData(chunk[0])) {
    var chunkPart = chunk.slice(0, 128)
    logger.error('close the connection with invalid head message, the remote ip is %s && port is %s && message is len=%s, %j', socket._socket.remoteAddress, socket._socket.remotePort, chunk.length, chunkPart);
    socket.close();
  }
