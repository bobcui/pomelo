...
Command.prototype.handle = function(socket) {
  ...
  if(!!socket.__hbTime__ && now - socket.__hbTime__ < this.heartbeatSec-3) {
    socket.disconnect('heartbeat too frequently')
    return
  }
  socket.__hbTime__ = now
  ...
  if(self.disconnectOnTimeout) {
    self.timeouts[socket.id] = setTimeout(function() {
      logger.debug('client %j heartbeat timeout.', socket.id);
      socket.disconnect();
    }, self.timeout);
  }
};
