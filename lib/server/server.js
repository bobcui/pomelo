...
var Server = function (app, opts) {
  this.opts = opts || {};
  this.handleEnqueue = this.opts.handleEnqueue || false;
  if (this.handleEnqueue) {
    ...
    this.handleQueue = [];
    this.handleAfterBeforeFilter = queueHandle;
  }
  else {
    this.handleAfterBeforeFilter = doHandleReal;
  }
...
pro.getHandleQueueLength = function() {
  ...
  return this.handleQueue.length; 
}

pro.handleQueueMsg = function() {
  var count = this.handleCountOnce;
if (this.debug) {
    console.warn('handleQueueMsg count=%s len=%s', count, this.handleQueue.length)
}
  while (this.handleQueue.length > 0) {
    var args = this.handleQueue.pop()
    doHandleReal.apply(null, args)
    count--
    if (!count) {
      return
    }
...
pro.start = function() {
  ...
  if (this.handleEnqueue) {
    setInterval(this.handleQueueMsg.bind(this), this.handleInterval)
  }
...
var doHandle = function(server, msg, session, routeRecord, cb) {
  ...
  var handle = function(err, resp, opts) {
    ...
    self.handleAfterBeforeFilter(server, msg, session, routeRecord, cb, err, resp, opts)
  };  //end of handle

  beforeFilter(false, server, msg, session, handle);
};

var queueHandle = function(server) {
  var args = Array.prototype.slice.call(arguments)
  server.handleQueue.push(args)
}

var doHandleReal = function(server, msg, session, routeRecord, cb, err, resp, opts) {
  var self = server;
